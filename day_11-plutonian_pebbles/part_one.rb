...
def stones_count_after_n_blinks(stone, blinks = 75)
  ...
  if stone == 0
    @cache[[1, blinks - 1]] = stones_count_after_n_blinks(1, blinks - 1)
  ...
  if length % 2 == 1
    @cache[[stone * 2024, blinks - 1]] = stones_count_after_n_blinks(stone * 2024, blinks - 1)
    return @cache[[stone * 2024, blinks - 1]]
  end

  @cache[[stone / 10 ** (length / 2), blinks - 1]] = stones_count_after_n_blinks(stone / 10 ** (length / 2), blinks - 1)
  @cache[[stone % 10 ** (length / 2), blinks - 1]] = stones_count_after_n_blinks(stone % 10 ** (length / 2), blinks - 1)
